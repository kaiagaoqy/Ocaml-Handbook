...
let check_closed expr =
  ...
  let rec check vars expr = 
   ...
   (* Func and Let *)
    ...
    (* Record *)
    | Record(expr) -> true
    | Select(label,expr) -> true
    | Append(expr1,expr2) -> true 

    | Raise(exnid,expr) -> true
    | Try(expr1,exnid,ident,expr2) -> true
...
let eval expr = 
  let rec evaluate expr = 
    ...
    |Let(x,e1,e2) -> evaluate @: subst e2 (evaluate e1) x
